...
Description of Problem:Created a table of the findings
'''
# create a function that calculates Earnigns for W=2
def buy_sell_w(column):
    money=100
    money_list=[]
    count=0
    for i in range(len(df_2)):
        R=df_2.iloc[i]['Return']
        if df_2.iloc[i]['Guess2_sign'] =='+':
            money=money+(R*money)
    ...
    return money_list
df_2['w_star']=0
df_2['w_star']=buy_sell_w(df_2['Guess2_sign'])

# create a function that calculates Earnigns for esemble
def buy_sell_e(column):
    money=100
    money_list=[]
    count=0
    for i in range(len(df_2)):
        R=df_2.iloc[i]['Return']
        if df_2.iloc[i]['ensemble'] =='+':
            money=money+(R*money)
            money_list.append(money)
        else:
            money_list.append(money)
    ...
    return money_list
df_2['e_cash']=0
df_2['e_cash']=buy_sell_e(df_2['ensemble'])

#create a list of what buy and hold would be
df_2['buy_and_hold']=0
hold=[100]*(len(df_2)-1)
df_2['buy_and_hold']=hold+[df_2['Return'].to_list()[-1]*100+100]
print(df_2)

plt.plot(df_2['Date'],df_2['w_star'],label='W*')
plt.plot(df_2['Date'],df_2['e_cash'],label='Ensemble')
plt.plot(df_2['Date'],df_2['buy_and_hold'],label='Buy and Hold')
plt.gca().xaxis.set_major_locator(mdates.DayLocator(interval=20))
plt.gcf().autofmt_xdate()
plt.xticks(rotation=45,fontsize=7)
plt.title("Profit over Two Years")
plt.xlabel("Dates")
plt.ylabel("Stock Profits")
